...
export const QrExperienceModal = ({ isOpen, onClose }) => {
  const [breathState, setBreathState] = useState('Nefes Al'); // Nefes Al, Tut, Nefes Ver, Tut
  const [secondsLeft, setSecondsLeft] = useState(4);
  const [isRunning, setIsRunning] = useState(false);
  const [activeSound, setActiveSound] = useState('alpha'); // alpha, beta
        ...
        setSecondsLeft((prev) => {
          if (prev <= 1) {
            ...
            setBreathState((currentPhase) => {
              if (currentPhase === 'Nefes Al (Inhale)') return 'Nefes Tut (Hold)';
              if (currentPhase === 'Nefes Tut (Hold)') return 'Nefes Ver (Exhale)';
              if (currentPhase === 'Nefes Ver (Exhale)') return 'Dinlen & Tut (Pause)';
              
              // Completed one 16-sec box cycle
              setCompletedSessions((c) => c + 1);
              return 'Nefes Al (Inhale)';
            });
            return 4;
          }
          return prev - 1;
      ...
      <div className="relative w-full max-w-lg glass-panel-glow bg-[#0B1020] border border-cyan-500/40 rounded-3xl p-6 sm:p-8 shadow-[0_0_50px_rgba(0,242,254,0.2)]">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-white flex items-center gap-2">
              <span>HEALTHY ESSENTIALS QR PORTALI</span>
              <span className="text-[10px] bg-cyan-950 text-cyan-300 px-2 py-0.5 rounded font-mono">CANLI DEMO</span>
            </h3>
            <p className="text-xs text-slate-400">Çalışanlarınızın tepsideki QR kod ile ulaştığı günlük bio-ritüel</p>
          </div>
        </div>
        ...
        <div className="bg-[#060913] p-6 rounded-2xl border border-slate-800 text-center space-y-4 mb-6">
          <div className="text-xs font-bold text-cyan-400 uppercase tracking-widest">
            2 Dk Parasempatik Box Breathing (Kutucuk Nefesi)
          </div>
          ...
          <div className="relative w-36 h-36 mx-auto flex items-center justify-center">
            ...
            <div className="text-center z-10 space-y-1">
              ...
              <div className="text-xs font-bold text-cyan-300">{isRunning ? breathState : 'Hazır mısınız?'}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            <button
              onClick={() => {
                if (!isRunning) {
                  setIsRunning(true);
                  setBreathState('Nefes Al (Inhale)');
                  setSecondsLeft(4);
                } else {
                  setIsRunning(false);
                }
            ...
            >
              ...
              <span>{isRunning ? 'Duraklat' : 'Nefes Egzersizini Başlat'}</span>
            </button>

            <button
              onClick={() => {
                setIsRunning(false);
                setBreathState('Hazır');
                setSecondsLeft(4);
              }}
          ...
          {completedSessions > 0 && (
            <div className="text-xs text-emerald-400 font-semibold animate-in fade-in">
              🎉 Harika! {completedSessions} tur tamamlandı. Nabzınız ve kortizolünüz dengeleniyor.
            </div>
          )}
        </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-3 mb-6">
          <div className="flex items-center justify-between text-xs font-bold text-slate-300">
            <span className="flex items-center gap-2">
              ...
              <span>Alfa Dalga Odak Sesi (Binaural Focus)</span>
            </span>
        ...
        <div className="p-3 rounded-xl bg-blue-950/40 border border-blue-900/60 flex items-start gap-3 text-xs text-slate-300">
          ...
          <div>
            <span className="font-bold text-white block">Günün Nöro-İpucu:</span>
            Sabah uyandıktan ilk 30 dakika içinde doğrudan güneş ışığına bakmak, melatonin üretimini zamanlayarak gece uyku kalitenizi %40 artırır.
          </div>
        </div>
